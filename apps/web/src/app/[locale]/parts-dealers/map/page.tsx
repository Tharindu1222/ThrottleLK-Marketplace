...
export async function generateMetadata({
  ...
  return pageMetadata({
    title: 'Parts shop map — spare & modified parts in Sri Lanka',
    description:
      'See approved ThrottleLK parts shops on the map and open their showrooms.',
    path: `/${locale}/parts-dealers/map`,
  });
}

export default async function PartsDealersMapPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  const dealers = await apiGet<MapDealer[]>('/api/v1/parts-dealers/map').catch(
    () => [] as MapDealer[],
  );
    ...
    <main className="flex min-h-[calc(100dvh-8.5rem)] flex-col">
      <div className="mx-auto flex w-full max-w-6xl flex-wrap items-center justify-between gap-3 px-6 py-4">
        <div>
          <h1 className="font-[family-name:var(--font-display)] text-2xl tracking-wide sm:text-3xl">
            {t(locale, 'partsDealersMap')}
          </h1>
        </div>
        <Link
          href={`/${locale}/dealers?type=parts`}
      ...
      {dealers.length === 0 ? (
        <div className="mx-auto flex w-full max-w-6xl flex-1 flex-col items-start justify-center gap-4 px-6 pb-16">
          <p className="text-muted">{t(locale, 'partsDealersMapEmpty')}</p>
          <Link
            href={`/${locale}/dealers?type=parts`}
