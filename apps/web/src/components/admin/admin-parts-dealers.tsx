...
import type { AdminPartsDealerRow, AdminUser, District } from '@/lib/admin-types';
...
export function AdminPartsDealers({ search = '' }: { search?: string }) {
  ...
  async function loadList(
    ...
    const { data, meta } = await apiGetWithMeta<AdminPartsDealerRow[]>(
      '/api/v1/admin/parts-dealers',
      {
        token: access,
        searchParams: {
  ...
  function openEdit(row: AdminPartsDealerRow) {
    setEditingId(row.id);
    setForm({
  ...
  async function onSubmit(e: FormEvent) {
    ...
    try {
      ...
      if (editingId) {
        await apiSend(`/api/v1/admin/parts-dealers/${editingId}`, {
          method: 'PATCH',
          token,
          body,
        });
        ...
        const created = await apiSend<AdminPartsDealerRow>('/api/v1/admin/parts-dealers', {
          token,
          body,
        });
  ...
  async function onDelete(id: string) {
    ...
    setError(null);
    try {
      await apiSend(`/api/v1/admin/parts-dealers/${id}`, { method: 'DELETE', token });
      await loadList(token, statusFilter, search, page);
    } catch (err) {
              ...
              {filtered.map((row) => (
                ...
                >
                  <td className="px-4 py-3">
                    <div className="flex flex-wrap items-center gap-2">
                      <Link
                        href={`/${locale}/admin/parts-dealers/${row.id}`}
          ...
          >
            ...
            {editingId ? (
              <div className="space-y-2">
                ...
                <DealerImageManager
                  dealerId={editingId}
                  apiBase="/api/v1/admin/parts-dealers"
                />
              </div>
            ) : (
              <p className="text-sm text-[var(--admin-muted)]">
